...
class TestTypeToStr:
    @pytest.mark.parametrize(
        "v, expected", [(create_var("a"), "a"), (create_par("b"), "b"), (create_array("arr", 2), "arr")]
    )
    ...
    @pytest.mark.parametrize(
        "array, pos, expected", [(create_array("z", 3), (1, 2, 3), "z[1, 2, 3]"), (create_array("g", 1), 2, "g[2]")]
    )
    ...
    @pytest.mark.parametrize(
        "collection, expected",
        [
            ((1, create_array("a", 2)[1, 2], 3, create_par("c")), "[1, a[1, 2], 3, c]"),
            ([1, create_par("c") + create_array("a", 2)[1, 2] + 3], "[1, ((c + a[1, 2]) + 3)]"),
        ],
    )
    ...
    @pytest.mark.parametrize(
        "gen, expected",
        [((x for x in create_array("z", 1)), "[z[0], z[1], z[2], z[3], z[4]]"), ((x for x in range(3)), "[0, 1, 2]")],
    )
